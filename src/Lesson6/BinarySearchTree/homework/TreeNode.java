package Lesson6.BinarySearchTree.homework;
...
public class TreeNode {
    ...
    public static TreeNode createRandomIntegerTree(int levelCount) {
        RANDOM_RANGE = 100;

        TreeNode root = new TreeNode();
        root.idata = (int) (Math.random() * 2 * RANDOM_RANGE - RANDOM_RANGE);

        while (depth(root) < levelCount) {
    ...
    public static void main(String[] args) {
        int treeCount = 20;
        int levelCount = 6;
        int maxLevelDiffForBalance = 1;
        ArrayList<TreeNode> trees = new ArrayList<>(20);

        int balancedTrees = 0;

        for (int i = 0; i < treeCount; i++) {
            trees.add(createRandomIntegerTree(levelCount));

            if (depth(trees.get(i)) - depthMin(trees.get(i)) <= maxLevelDiffForBalance) {
                balancedTrees++;
            }
        }

        System.out.println("Balanced trees: " + balancedTrees + "/" + treeCount
                                              + String.format("   (%.1f", (double)balancedTrees/treeCount*100) +"%)");

        System.out.println("MIN DEPTH = " + depthMin(trees.get(0)));
        System.out.println("MAX DEPTH = " + depth(trees.get(0)));
        trees.get(0).inOrderInteger(", ");
        drawTree(trees.get(0));
    }
    ...
    public static int depthMin(TreeNode n) {
        if (n == null)
            return 0;
        n.depth = 1 + Math.min(depth(n.left), depth(n.right));
        return n.depth;
    }
